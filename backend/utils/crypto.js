            ...
            const decrypted = crypto.privateDecrypt(
                {
                    ...
                    oaepHash: 'sha256',
                    mgf1Hash: 'sha256', // Explicitly match Web Crypto default
                },
                buffer
            );
            const decryptedStr = decrypted.toString('utf8');
            console.log(`🔐 RSA Decryption Successful (Len: ${decryptedStr.length})`);
            return decryptedStr;
        } catch (error) {
            console.error("RSA Decryption Failed:", error.message);
            if (error.message.includes('oaep decoding error')) {
                console.error("Detailed: OAEP Hash mismatch or corrupted payload. Check MGF1/OAEP parity.");
            }
